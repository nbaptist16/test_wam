...
def talker():
    ...
    # params
    # number_of_joints = 8    # wam = 7 dof + 1 dof from track
    # number_of_joints = 16    # wam = 7 dof + 1 dof from track + 8 joints from bhand
    number_of_joints = 19
    ...
    # Create  J0 through J18
    for joint_number in range (number_of_joints):  
       all_sets_of_joint_coordinates.joint_names.append("J%i"%joint_number)

    # time at which to take initial positions
    time_for_move = .01
    # number_of_boring_points = 100 # re-added
    # number_of_boring_points = 50  ->  850
    number_of_boring_points = 10 #  ->  170
    ...
    angls = np.matrix([j0pts, j1pts, j2pts, j3pts, j4pts, j5pts, j6pts, j7pts, j8pts, j9pts, j10pts, j11pts, j12pts, j13pts, j14pts, j15pts, j16pts, j17pts, j18pts])

    for i in range (number_of_boring_points): 
    # for i in range (0,2):
    #    for joint_number in range (1, number_of_joints + 1):
        for joint_number in range (1, number_of_joints):
        # for joint_number in range (number_of_joints): # makes it mad
            if ((joint_number != 8) and (joint_number != 9)):
                if (i == 0):
                    joint_coord = angls[joint_number]
                    # one_set_of_joint_coordinates.positions.append(angls[joint_number])
